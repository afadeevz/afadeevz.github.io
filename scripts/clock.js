...
class Clock {
    ...
    drawHand(angle, length, width, color) {
        ...
        this.context.beginPath();
        let x = d / 2 - 25 * Math.cos(angle);
        let y = d / 2 + 25 * Math.sin(angle);
        this.context.moveTo(x, y);
        x = d / 2 + length * Math.cos(angle);
        y = d / 2 - length * Math.sin(angle);
    ...
    draw(time) {
        ...
        let r = d / 2 - 30;
        let t = ((time.getHours() * 60) + time.getMinutes()) * 60 + time.getSeconds();
        let hours = t / 60 / 60;
        let minutes = t / 60 % 60;
        let seconds = t % 60 % 60;
        ...
        let width = 5;
        let color = "#000000";
        let angle = Math.PI / 2 - hours / 24 * 2 * Math.PI;
        this.drawHand(angle, length, width, color);
